#!/bin/bash
cp -r templates/deploy-template.conf deployment/test/deploy.sh #copy setup_template file to setup.conf file
...
export DESTINATION_FILE=deployment/test/deploy.sh # folder where the distribution's YAML files are to be found
